...
for i, line in enumerate(lines, 1):
    for pattern in incorrect_patterns:
        if re.search(pattern, line):
            ...
            print(f"  Error: Incorrect default filter usage")
            print(f"  Should be: {{ variable|default:\"default_value\" }}")
            print()
            found = True
...
if not found:
    ...
    for i, line in enumerate(lines, 1):
        if "|default" in line and ":" not in line:
            print(f"Line {i}: {line.strip()}")
            print(f"  Warning: Possible incorrect default filter usage")
            print(f"  Should be: {{ variable|default:\"default_value\" }}")
            print()
            found = True
        elif "|default:" in line and ("|default: " in line or "|default:  " in line):
            print(f"Line {i}: {line.strip()}")
            print(f"  Warning: Possible space after colon in default filter")
            print(f"  Should be: {{ variable|default:\"default_value\" }}")
            print()
            found = True
