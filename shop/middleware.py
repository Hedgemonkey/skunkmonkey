...
class CheckoutDebugMiddleware(MiddlewareMixin):
    ...
    def process_response(self, request, response):
        ...
        if request.path == '/shop/checkout/' and response.status_code == 302:
            ...
            logger.error(f"CHECKOUT REDIRECT DETECTED: Redirecting to {response.url}")
            
            # Log cart information
            if hasattr(request, 'cart'):
                cart = request.cart
                logger.error(f"Cart info: Items count: {cart.items.count()}, Total: {cart.total_price}")
                
                # Log individual items
                for item in cart.items.all():
                    logger.error(f"Cart item: {item.product.name}, Quantity: {item.quantity}")
            else:
                logger.error("Cart attribute not found on request")
                
            # Log Stripe configuration
            from django.conf import settings
            logger.error(f"Stripe public key present: {'Yes' if hasattr(settings, 'STRIPE_PUBLISHABLE_KEY') and settings.STRIPE_PUBLISHABLE_KEY else 'No'}")
            logger.error(f"Stripe secret key present: {'Yes' if hasattr(settings, 'STRIPE_SECRET_KEY') and settings.STRIPE_SECRET_KEY else 'No'}")
