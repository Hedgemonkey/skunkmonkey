...
class Order(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.grand_total and self.total_price is not None:
            self.grand_total = decimal.Decimal(
                self.total_price) + decimal.Decimal(self.shipping_cost)
    ...
    # No need for billing_full_name property as we now use billing_name
    # directly
...
class RecentlyViewedItem(models.Model):
    ...
    class Meta:
        ...
        # Remove the unique_together constraint to allow multiple views of the
        # same product
        indexes = [
            models.Index(
                fields=[
                    'user',
                    'product'],
                name='shop_recent_user_prod_idx'),
        ]
