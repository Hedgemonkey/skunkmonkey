...
def is_client_secret_valid(request, cart):
    ...
    if not is_valid:
        logger.debug(f"Cart signature mismatch: stored={stored_signature}, current={current_signature}")
...
def store_stripe_session_data(request, cart, client_secret):
    ...
    logger.debug(f"Stored new client_secret and cart_signature: {cart_signature}")
