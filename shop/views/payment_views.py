...
class CreatePaymentIntentView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            return JsonResponse({'error': 'Payment system configuration error'}, status=500)
        ...
        try:
            ...
            logger.info(f"Created payment intent {intent.id} for {request.user.username if request.user.is_authenticated else 'AnonymousUser'}")
    ...
    def get_stripe_api_key(self):
        ...
        try:
            ...
            if api_key and api_key.secret:
                logger.debug(f"Using API key from database: {getattr(api_key, 'name', 'unknown')}")
...
def reset_payment_intent(request):
    ...
    try:
        ...
        if not stripe_api_key:
            logger.error("No Stripe API key found")
            messages.error(request, "Payment system configuration error. Please try again later.")
    ...
    try:
        ...
        request.session['client_secret'] = intent.client_secret
        logger.info(f"Created new payment intent {intent.id} after reset for {request.user.username if request.user.is_authenticated else 'AnonymousUser'}")
        
        messages.success(request, "Your payment session has been refreshed. You can now proceed with checkout.")
        return redirect('shop:checkout')
        
    except stripe.error.StripeError as e:
        logger.error(f"Stripe error creating new payment intent during reset: {e}")
        messages.error(request, f"Error with payment system: {str(e)}")
        return redirect('shop:checkout')
    except Exception as e:
        logger.error(f"Unexpected error creating payment intent during reset: {e}")
