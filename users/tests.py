...
class UsersViewsTestCase(TestCase):
    ...
    def test_users_contact_post(self):
        response = self.client.post(reverse('users:users_contact'), {
            'email': 'testuser@example.com',
...
class UsersModelsTestCase(TestCase):
    ...
    def test_address_str(self):
        self.assertEqual(str(self.address), 'testuser - 123 Test Street, 12345')
...
class UsersFormsTestCase(TestCase):
    ...
    def test_custom_add_email_form_invalid(self):
        form = CustomAddEmailForm(data={
            'email': 'newemail@example.com',
            'email2': 'differentemail@example.com'  # Emails don't match
        })
        self.assertFalse(form.is_valid())
...
class UsersSignalsTestCase(TestCase):
    ...
    def test_update_user_email_signal(self):
        ...
        update_user_email(sender=None, request=None, email_address=email_address)
