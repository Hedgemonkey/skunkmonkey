...
def fix_default_filters(content):
    ...
    # Pattern 2: {{ var|default value }} without quotes -> {{ var|default:"value" }}
    pattern2 = re.compile(r'(\{\{\s*[^}|]+)\|default\s+([^"}][^}]*)\}\}')
    content = pattern2.sub(r'\1|default:"\2"\}\}', content)
    
    # Pattern 3: {{ var|default: value }} space after colon -> {{ var|default:"value" }}
    ...
    # This is tricky because we need to avoid matching already correct usage like {{ var|default:"value" }}
